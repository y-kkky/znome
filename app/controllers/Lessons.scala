...
//  val perc = ("%1.2f".format((ra * 100)/max)).toDouble
    ...
    Redirect(routes.Lessons.dailyStatGet(current_date, typ)).flashing("success" -> s"Ваш результат: ${perc} балів зі 100")
  }
  ...
  def dailyStatRedirect(typ: Int) = withUser{ user => implicit request =>
    ...
    if(dateArray.length != 3){
      ...
      Redirect(routes.Lessons.dailyStatGet(resultDate, typ))
    }
				 }
